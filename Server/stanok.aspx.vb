...
Partial Class stanok
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If SN <> "" Then
            w = w + " and OBOR.INVN='" + SN + "' "
        Else
            w = w + " and OBOR.INVN is null"
        End If
        ...
        If dt.Rows.Count > 0 Then
            AddData(dt2, dt.Rows(i), "zavod", "Филиал")
            AddData(dt2, dt.Rows(i), "ceh", "Цех")
            AddData(dt2, dt.Rows(i), "invn", "Инвн.")
            AddData(dt2, dt.Rows(i), "smen", "Cменность")
            AddData(dt2, dt.Rows(i), "naim", "Наименование")
            AddData(dt2, dt.Rows(i), "mgroup", "Группа")
            AddData(dt2, dt.Rows(i), "krit", "Критичность")
            AddData(dt2, dt.Rows(i), "kzv", "Кзв", 1)
            AddData(dt2, dt.Rows(i), "kdv", "Кдв", 1)
            AddData(dt2, dt.Rows(i), "kpv", "Кпв", 1)
            AddData(dt2, dt.Rows(i), "kchv", "Кчв", 1)
            AddData(dt2, dt.Rows(i), "oee", "ОЕЕ", 1)
            AddData(dt2, dt.Rows(i), "teepo", "ТЕЕРо", 1)
            AddData(dt2, dt.Rows(i), "mvo", "Мво", 1)
            AddData(dt2, dt.Rows(i), "teepa", "ТЕЕРа", 1)
            AddData(dt2, dt.Rows(i), "mva", "Мва", 1)
            AddData(dt2, dt.Rows(i), "neterr", "Сетевая ошибка")
            AddData(dt2, dt.Rows(i), "ner_full", "Нерабочее время")
            AddData(dt2, dt.Rows(i), "ned_full", "Недоступное время (потери доступности)")
            AddData(dt2, dt.Rows(i), "vspom_full", "Вспомогательные операции")
            AddData(dt2, dt.Rows(i), "nez_full", "Незапланированные операции")
            AddData(dt2, dt.Rows(i), "so0", "Ожидание ввода оператора, сбой")
            AddData(dt2, dt.Rows(i), "so2", "Праздники и выходные")
            AddData(dt2, dt.Rows(i), "so3", "Обед")
            AddData(dt2, dt.Rows(i), "so4", "Плановый ремонт централизованными службами")
            AddData(dt2, dt.Rows(i), "so5", "Аварийный ремонт централизованными службами")
            AddData(dt2, dt.Rows(i), "so6", "Отсутствие задания")
            AddData(dt2, dt.Rows(i), "so7", "Отсутствие материала, заготовок, деталей")
            AddData(dt2, dt.Rows(i), "so8", "Отсутствие инструмента, оснастки, вспомогательного оборудования")
            AddData(dt2, dt.Rows(i), "so9", "Отсутствие крана, транспорта")
            AddData(dt2, dt.Rows(i), "so10", "Отсутствие оператора в связи с необеспеченностью")
            AddData(dt2, dt.Rows(i), "so11", "Неявка оператора (б\лист, отпуск, командировка)")
            AddData(dt2, dt.Rows(i), "so12", "Отсутствие энергоносителей")
            AddData(dt2, dt.Rows(i), "so13", "Отсутствие сотрудника ОТК")
            AddData(dt2, dt.Rows(i), "so14", "Отсутствие конструктора, технолога или ожидание его решения")
            AddData(dt2, dt.Rows(i), "so15", "Естественные надобности")
            AddData(dt2, dt.Rows(i), "so16", "Ознакомление с работой, документацией и инструктаж")
            AddData(dt2, dt.Rows(i), "so17", "Переналадка оборудования, получение инструмента до начала работы, снятие\\сдача по окончании работы")
            AddData(dt2, dt.Rows(i), "so18", "Работа с управляющей программой")
            AddData(dt2, dt.Rows(i), "so19", "Установка, выверка, снятие детали")
            AddData(dt2, dt.Rows(i), "so20", "Изменение режимов, смена инструмента, приспособления")
            AddData(dt2, dt.Rows(i), "so21", "Контроль на рабочем месте")
            AddData(dt2, dt.Rows(i), "so22", "Уборка, осмотр оборудования, чистка\смазка станка")
            AddData(dt2, dt.Rows(i), "so23", "Сборочные операции")
            AddData(dt2, dt.Rows(i), "so24", "Работа по карте несоответствий")
            AddData(dt2, dt.Rows(i), "so25", "Нерабочее время по графику согласно сменности")
